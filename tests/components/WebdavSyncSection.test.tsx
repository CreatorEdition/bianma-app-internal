...
describe("WebdavSyncSection", () => {
  ...
  it("saves sync scope selection and shows snapshot boundary notes", async () => {
    renderSection(baseConfig);

    expect(
      screen.getByText("settings.webdavSync.scope.snapshotNotice"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("settings.webdavSync.manualSnapshotNotice"),
    ).toBeInTheDocument();

    const mcpScopeCard = screen
      .getByText("settings.webdavSync.scope.options.mcp.label")
      .closest("label");
    expect(mcpScopeCard).not.toBeNull();

    fireEvent.click(
      mcpScopeCard!.querySelector('input[type="checkbox"]') as HTMLInputElement,
    );
    fireEvent.click(
      screen.getByRole("button", { name: "settings.webdavSync.save" }),
    );
  ...
  it("fetches remote info and downloads after confirmation", async () => {
    ...
    expect(
      screen.getByText("settings.webdavSync.confirmDownload.scopeNotice"),
    ).toBeInTheDocument();
