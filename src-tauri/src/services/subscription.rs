...
///
/// 这是 Gemini CLI 上游公开源码使用的 installed application OAuth client。
/// 上游注释明确说明该 client secret 可随安装应用源码保存，不按服务端密钥处理。
/// 来源：google-gemini/gemini-cli `packages/core/src/code_assist/oauth2.ts`。
