...
`bianma-app` 是 Bianma 的唯一正式开源主仓。后续源码、公开协作、Release、Updater 与二进制分发默认都以本仓为准；历史 `bianma-app-product` 仅作为迁移源与待归档目录。
...
`src-tauri/crates/routing-core` 是独立的纯 Rust Stage-first 规划器。它使用不可变内存快照生成固定容量的 `A -> B -> C` 计划，以固定大小 ReplayGate 区分“零写出”“受信执行前拒绝”和 `DeliveryUnknown`，再由所有权驱动的线性 Coordinator 保证每条逻辑请求链最多只有一个活跃 Attempt，并且只允许从当前位置推进到下一 Stage。普通 429/503、`Retry-After` 或错误文本不能自行授权重放；已写出不明、首个有效语义事件或下游提交后必须停止。核心仍不访问数据库、文件或网络，不等待、不启动后台线程，也不持有账户或凭据；实际 HTTP、冷却、同 Target 等待、换 Key、Secret 和 ContextPipeline 继续由后续独立切片接入。
