...
- ✅ 已完成（2026-08-10）：完成 T0「Transport 交接保守封口」。`SelectedAttempt` 不再暴露裸 `AttemptTracker` 或 crate-wide 直接终结出口，只能按值转换为线性 `TransportHandoffAttempt`；包装器仅提供受控写入、响应、下游提交及密封 Reporter 操作。交接后任意无收据的 Timeout、429、503、连接重置或自由错误都在终结前封为 `DeliveryState::Unknown`，不会生成下一 Permit；已写或下游提交仍优先不可重放。唯一例外是既有一次性、密封且绑定同一 Attempt/Target/快照的 `TrustedPreExecutionRejection`，仍由 Tracker 逐项校验后保留 `PreExecutionRejected` 重试语义；错配收据继续 fail closed 为 `Unknown`。不完整成功返还同一 handoff 包装器，取消和 Drop 保持 C2 Lease 生命周期。C2 本地容量拒绝仍发生在交接前并零发送 A→B。89 个 `routing-core` 调试单测、release 模式 Lease/Transport 定向 11 个单测、fmt、Clippy `-D warnings`、严格 rustdoc、零正常依赖与 `git diff --check` 通过；独立 Terra 对 `bee2e35` 与修订 `7d694fc` 均 ACCEPT。无真实 Transport、HTTP/Retry-After 解析、Secret、Vault、数据库、async、线程、队列、时钟、adapter 注册表或公开 API。
- ✅ 已完成（2026-08-10）：完成 C2「PriorityFailover 与原子三资源 Lease」。`SelectionLeaseRegistry` 按值独占 C1 `SelectionRuntimeLayout`，仅在激活时借同代 `CompiledRoutingSnapshot` 校验策略；`WeightedLeastInflight`、`RoundRobinCompat` 明确拒绝，不能降级为 Priority。选择入口消费 `AttemptPermit + AccountSelectionEligibility`，按优先层升序、层内声明顺序选择；当前层容量满会继续尝试下一优先层。每次候选以 `Account → Credential → QuotaGroup ID 升序` 取得固定原子槽，任一失败完整逆序回滚；获取单槽最多 CAS 8 次，竞争过高保守返回本地容量不足。Lease 归还使用 checked-CAS，零值不回绕；为保证精确回收，极端竞争时归还允许 CAS 重试，不能将“8 次上限”外推到归还路径。成功 Lease 位于 `SelectedAttempt` 外层，Drop 只回收容量。容量/动态无候选会生成密封本地拒绝，Coordinator 直接以零延迟推进 A→B，不调用 RetryGate、不写 Health/429、不伪造 `FailureClass`；快照/Target/provenance 或未实现策略失配会密封本地停止并 fail closed。成功、失败、取消均在线性出口释放 Lease，成功仍只能经 C2-S0 的完整响应 token 终结。独立 Terra 实际 diff 审计最终 ACCEPT；88 个 `routing-core` 调试单测、release 模式 Lease 定向 10 个单测、fmt、Clippy `-D warnings`、严格 rustdoc、零正常依赖、`git diff --check` 与尺寸门禁通过。无网络、Transport、Secret、数据库、async、后台线程、排队/等待、受信 429 冷却或旧 proxy。
- ✅ 已完成（2026-08-10）：完成 C1-AC「账户、凭据与额度组统一静态布局」。唯一 `SelectionRuntimeDefinitions → SelectionRuntimeLayout` 激活入口会精确覆盖全部可达 Account、Credential 与 `QuotaGroupId` 的全局唯一正整数并发上限，空/超限/重复/缺失/多余定义均拒绝。`binding_for` 仅为同一快照、当前 Target 的 Selector、对应 Unit 与实际 Member 生成零分配三资源 binding；binding 私有保存快照实例、唯一 TargetId 与完整 Member 来源，并提供受控 provenance/尝试目标校验，禁止同快照 Target A/B 混配。75 个 `routing-core` 单测、全工作区 fmt、Clippy、严格 rustdoc、零正常依赖与 diff 检查均通过；独立 Terra 初审发现并阻断 provenance 丢失，修订 `669058a` 后复审 ACCEPT，无剩余 BLOCKING/NON-BLOCKING。C1-AC 不产生选中身份、Lease、Registry、inflight 写入、策略、Health/429、Attempt/Replay、Secret 或网络；C2 必须消费该 provenance 后才可实现原子 Lease。
