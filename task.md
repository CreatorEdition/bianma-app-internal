...
- ✅ 已完成（2026-08-07）：实现 `routing-core v2` 第二个纯 Rust 安全切片：封闭 `RequestClassifier` 与 Local/BoundDeployment/Routed 三路 typestate。`ingress-contract` 已补齐 protocol/registry digest、receiver 后置 typestate、Managed/Gateway/Capability 无构造权只读视图及 Capability Site/TrustTier 签名绑定；receiver `accept`、accepted typestate 与 Normalizer 正文读取均保持 crate-private，`routing-core` package 仅暴露同一类型身份 facade。分类器完成 Local/Managed/Gateway/Capability/ExactUpstream 闭集、Normalizer 二次全等、Context/Bound 双门禁、逐请求 grant 绑定、固定单 Target/Account/Credential、单次 Attempt 与禁止 fallback。本地 stable 环境已通过 27/27 ingress 单测、5/5 ingress compile-fail doctest、13/13 routing 单测、8/8 routing compile-fail doctest、Clippy、rustfmt、missing-docs 与边界审计；远端 Ingress Contract Checks 与 Rust 1.85.0 MSRV CI（run 31318465181）均已通过。仍不接现有 `proxy/**`、生产 Adapter、RoutingSnapshot 编译器、Planner、429/retry/fallback、Vault、SecretResolver、Transport、UI、迁移或切流。
- ✅ 已完成（2026-08-07）：实现 `routing-core v2` 首个纯 Rust 安全切片 `ingress-contract`。已建立非虚拟 Cargo workspace、原始请求/Operation 绑定、授权 bundle 整体 HMAC、Managed/GatewayOnly/LocalOperation/CapabilityScoped 四个封闭 verifier 入口、字段私有且不可反序列化的 Verified 类型、有界原子 nonce 消费、runtime seal 与独立 stable/MSRV CI；24/24 单元测试、2/2 compile-fail doctest、Clippy `-D warnings`、rustfmt、`missing_docs`、TypeScript、变更文档/CI Prettier、敏感信息与迁移 denylist 门禁均通过，三路最终红队未发现剩余 P0/P1。本轮仍未接入现有生产代理，未实现 ContextPipeline 签发端、Vault、Planner、Transport、UI、迁移或切流，也不声明 Managed Execute 已可启用。
