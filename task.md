...
- 🔄 进行中（2026-08-13）：重做 RT-002 轻量控制面。默认路径已经固定为“概览 → 上游 → 路由 → 用量”，首次上游仅要求 API 地址、API Key 与默认模型；当前正在补齐全新安装首次上游自动成为当前供应商，以及部分同步失败可见、可重试的端到端闭环。本切片不修改旧 Forwarder，不新增后台轮询。
