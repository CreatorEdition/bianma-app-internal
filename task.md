...
- 🔄 进行中（2026-08-10）：新增 C0「账户选择请求与动态 eligibility 合同」，为未来同站多账户/多 Credential 的安全实际选择固定输入边界。Selector 将携带不可变 revision 与 affinity salt；Session 只接受宿主已经 HMAC 化的别名，核心不接收原始 Session Key 或实现 HMAC。Eligibility 必须与同一 `ResolvedRouteTarget` / `RoutingSnapshot` 绑定，并以 16 位 Unit/Member 位图保守表达动态资格；越界位、未知 Unit、Member 未获其 Unit 允许、空 Unit 或跨快照均 fail closed。同一 QuotaSelectionUnit 混用 priority tier 将在编译期拒绝，保证未来 Unit-first Priority 语义唯一。本切片不执行选择、不创建 Lease、不更新 Health/429、不修改 Attempt/Replay、不读取 Secret/网络/时钟或旧 proxy。
