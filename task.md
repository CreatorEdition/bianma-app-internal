...
- 🔄 进行中（2026-08-10）：启动 C2 实施前门禁「PriorityFailover 与原子三资源 Lease」。经独立审计确认 C1 provenance 可被 Permit 安全消费，但当前 `AttemptTracker` 只有失败完成路径，直接丢弃也不会清除 Coordinator active 状态；因此先完成同一 C2 内的 C2-S0「私有成功完成 typestate 合同」，再实现 Registry/Lease。C2-S0 以 crate-private `CompletedAttemptTracker` 消费且封闭已完整提交的 Tracker；只有它可生成 Success Completion，使 Coordinator 清除 active 并返回私有 Completed。原 Tracker 只保留失败/取消出口，不得用 `FailureClass` 伪装成功、不得新增公开 `SendPhase` variant，也不实现网络/Transport。之后才固定 Account + Credential + Unit 全部 QuotaGroup 的确定性原子获取、失败回滚和 Drop/成功/取消释放合同。`WeightedLeastInflight`、`RoundRobinCompat`、受信 429 冷却、排队/等待、真实转发与旧 proxy 均不在此任务中。
