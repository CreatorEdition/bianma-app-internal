...
- ✅ 已完成（2026-08-13）：收口 RT-002 默认页面的信息层级。上游默认页不出现客户端切换器；路由页以单一聚合状态呈现统一接入结果，不再把 Claude/Codex/Gemini 逐项状态作为主内容；逐客户端渠道、模型映射、Header 与兼容参数仅保留在“客户端例外配置”高级入口。本切片未新增依赖、轮询或后端契约。
