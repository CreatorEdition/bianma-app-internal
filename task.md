...
- ✅ 已完成（2026-08-09）：完成最小 `routing-core` 领域核心切片。保留 Tauri/React 外壳，独立使用零第三方依赖的纯 Rust crate，不修改旧 `src-tauri/src/proxy/**`；实现固定上限 16 个候选、Priority/RoundRobin/LeastPenalty 计划、快照版本绑定与保守 ReplayGate。仅允许传输层 `NotSent` 或受信适配器证明的 `PreExecutionRejected` 推进；普通 HTTP 429 不自动重放。11 个单测、Clippy、rustdoc 通过；RoutePlan ≤160 bytes，运行时代码 ≤520 行；16 候选 release 微基准 100,000 次约 85 ns/plan。本切片尚未接入生产转发链，不承载 ContextPipeline。
