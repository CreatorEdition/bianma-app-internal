...
- 🔄 进行中（2026-08-10）：C2「PriorityFailover 与原子三资源 Lease」已完成实现，等待最终独立实际 diff 审计。`SelectionLeaseRegistry` 按值独占 C1 `SelectionRuntimeLayout`，仅在激活时借同代 `CompiledRoutingSnapshot` 校验策略；`WeightedLeastInflight`、`RoundRobinCompat` 明确拒绝，不能降级为 Priority。选择入口消费 `AttemptPermit + AccountSelectionEligibility`，按优先层升序、层内声明顺序选择；当前层容量满会继续尝试下一优先层。每次候选以 `Account → Credential → QuotaGroup ID 升序` 取得固定原子槽，任一失败完整逆序回滚；每个槽 CAS 最多 8 次，竞争过高保守返回本地容量不足，防止无界自旋抬高 CPU。成功 Lease 位于 `SelectedAttempt` 外层，Drop 只回收容量。容量/动态无候选会生成密封本地拒绝，Coordinator 直接以零延迟推进 A→B，不调用 RetryGate、不写 Health/429、不伪造 `FailureClass`；快照/Target/provenance 或未实现策略失配会密封本地停止并 fail closed。成功、失败、取消均在线性出口释放 Lease，成功仍只能经 C2-S0 的完整响应 token 终结。87 个 `routing-core` 单测、fmt、Clippy `-D warnings`、严格 rustdoc、零正常依赖、`git diff --check` 与尺寸门禁通过；独立 Terra 审计尚未完成。无网络、Transport、Secret、数据库、async、后台线程、排队/等待、受信 429 冷却或旧 proxy。
