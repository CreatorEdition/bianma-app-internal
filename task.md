...
- 🔄 进行中（2026-08-10）：启动 C2 实施前门禁「PriorityFailover 与原子三资源 Lease」。本阶段只验证现有 `AttemptPermit`/`AttemptTracker` 生命周期能否安全承接同一快照、同一 Target 的 binding provenance，并固定 Account + Credential + Unit 全部 QuotaGroup 的确定性原子获取、失败回滚和 Drop/成功/取消释放合同。`WeightedLeastInflight`、`RoundRobinCompat`、受信 429 冷却、排队/等待、真实转发与旧 proxy 均不在此任务中；若 Permit 绑定或成功闭环不足，则先补最小前置合同，禁止伪造执行完成。
