...
- 🔄 进行中（2026-08-10）：推进 C2「PriorityFailover 与原子三资源 Lease」。已完成 C2-S0「私有成功完成合同」：`AttemptTracker::into_response_completed` 只有在上游响应已受控观察、已写入字节且下游已提交时才消费 Tracker 并签发密封 Success token；token 仅含私有 Coordinator/Attempt 身份，不能由裸 Permit 或其他 crate 内模块构造，Coordinator 校验后清除 active、终结请求且不进入 RetryGate。原 Tracker 只保留失败/取消出口，不用 `FailureClass` 伪装成功、未改公开 `SendPhase`。78 个 `routing-core` 单测、fmt、Clippy、严格 rustdoc、零正常依赖、diff 检查和 377/380 Attempt 运行时代码门禁通过；独立 Terra 两轮实际审计先阻断再 ACCEPT。C2 Registry/Lease/选择仍未实现：之后才固定 Account + Credential + Unit 全部 QuotaGroup 的确定性原子获取、失败回滚和 Drop/成功/取消释放合同。`WeightedLeastInflight`、`RoundRobinCompat`、受信 429 冷却、排队/等待、真实转发与旧 proxy 均不在此任务中。
