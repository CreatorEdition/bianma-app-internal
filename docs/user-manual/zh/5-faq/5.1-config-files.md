...
该目录名沿用历史兼容标识；迁移背景见 [5.5 迁移与旧协议兼容](./5.5-migration-compatibility.md)。
...
| 字段 | 说明 |
|------|------|
| `models.providers` | 供应商配置（映射为 bianma-app 的"供应商"） |
| `env` | 环境变量配置 |
...
bianma-app 修改配置时的优先级：

1. **bianma-app 数据库** - 单一事实源 (SSOT)
...
- CLI 工具的配置文件（会被 bianma-app 回填）
- bianma-app 的 `settings.json`
...
bianma-app v3.7.0 从 JSON 文件迁移到 SQLite：
