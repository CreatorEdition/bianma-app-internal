...
3. bianma-app 调用供应商的 `/v1/models` 端点获取模型列表
4. 从按供应商分组的下拉菜单中选择模型
...
> 💡 认证类型由 bianma-app 自动检测（PackyCode API 代理 / Google OAuth / 通用 API Key），无需手动配置。
...
bianma-app 支持两种方式导入供应商配置：

### 方式一：深度链接导入

通过 `bianma://` 协议链接一键导入；如需查看旧 scheme 与旧命名的兼容范围，请见[迁移兼容说明](../5-faq/5.5-migration-compatibility.md)。

1. 点击或访问深度链接
2. bianma-app 自动打开并显示导入确认
3. 预览配置信息
4. 点击「确认导入」

**获取深度链接**：
- 从他人分享获取
- 使用仓库内的[本地生成页面](../../../../deplink.html)创建
...
> ⚠️ **注意**：导入会覆盖现有数据库，建议先导出当前配置作为备份。导出文件名可能仍沿用历史兼容前缀 `cc-switch-export-{时间戳}.sql`；迁移背景见 [5.5 迁移与旧协议兼容](../5-faq/5.5-migration-compatibility.md)。
