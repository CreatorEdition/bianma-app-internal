...
3. bianma-app がプロバイダーの `/v1/models` エンドポイントを呼び出してモデルリストを取得
...
> 認証タイプは bianma-app が自動的に検出します（PackyCode API プロキシ / Google OAuth / 汎用 API Key）。手動設定は不要です。
...
bianma-app は 2 つの方法でプロバイダー設定をインポートできます：

### 方法 1：ディープリンクでインポート

`bianma://` プロトコルリンクでワンクリックインポートできます。旧 scheme や旧命名の互換範囲は [移行互換ガイド（ZH）](../../zh/5-faq/5.5-migration-compatibility.md) を参照してください。

1. ディープリンクをクリックまたはアクセス
2. bianma-app が自動的に開き、インポート確認を表示
3. 設定情報をプレビュー
4. 「インポートを確認」をクリック
...
> **注意**：インポートは既存のデータベースを上書きするため、事前に現在の設定をエクスポートしてバックアップすることをお勧めします。エクスポートファイルは歴史的な互換プレフィックス `cc-switch-export-{タイムスタンプ}.sql` を引き続き使用する場合があります。移行背景は [移行互換ガイド（ZH）](../../zh/5-faq/5.5-migration-compatibility.md) を参照してください。
