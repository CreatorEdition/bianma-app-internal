...
## bianma-app のデータストレージ

### ストレージディレクトリ

デフォルトの場所：`~/.cc-switch/`

このディレクトリ名は歴史的な互換識別子です。移行背景は [移行互換ガイド（ZH）](../../zh/5-faq/5.5-migration-compatibility.md) を参照してください。
...
| フィールド | 説明 |
|------|------|
| `models.providers` | プロバイダー設定（bianma-app の「プロバイダー」にマッピング） |
| `env` | 環境変数設定 |
...
bianma-app が設定を変更する際の優先順位：

1. **bianma-app データベース** - 単一事実源 (SSOT)
...
- CLI ツールの設定ファイル（bianma-app がバックフィルする）
- bianma-app の `settings.json`
...
bianma-app v3.7.0 で JSON ファイルから SQLite に移行しました：
