...
bianma-app automatically detects conflicts between system environment variables and app configurations, preventing configurations from being unexpectedly overridden.
...
Found X environment variables that may conflict with bianma-app configuration
[Expand] [Dismiss]
```
...
3. bianma-app will automatically back up and remove the selected variables
...
The backup path name above is retained as a legacy compatibility identifier. See the [migration compatibility guide (ZH)](../../zh/5-faq/5.5-migration-compatibility.md).
...
If you prefer not to use bianma-app to remove variables, you can handle them manually:
...
- bianma-app provider configurations being overridden
- API requests being sent to the wrong endpoint
- Using the wrong API key

## Best Practices

1. **Use bianma-app to manage configurations**: Avoid setting API keys in system environment variables
