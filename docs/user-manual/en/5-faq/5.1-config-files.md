...
## bianma-app Data Storage

### Storage Directory

Default location: `~/.cc-switch/`

This directory name keeps a legacy compatibility identifier. See the [migration compatibility guide (ZH)](../../zh/5-faq/5.5-migration-compatibility.md).
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to bianma-app's "providers") |
...
bianma-app's priority when modifying configurations:

1. **bianma-app Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by bianma-app)
- bianma-app's `settings.json`
...
bianma-app v3.7.0 migrated from JSON files to SQLite:
