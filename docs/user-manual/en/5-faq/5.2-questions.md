...
The public release pipeline is still pending final signing and notarization gates. Before installing a new macOS build, follow the checksum, signing, and notarization status published with that specific GitHub Release.

If you encounter issues, obtain the latest installer from the distribution channel currently approved by your team or operator. The public `bianma-app` repository is not the current formal binary release repository.
...
2. Add bianma-app to your antivirus software's whitelist
...
./<downloaded-appimage> --no-sandbox
```

Current Linux release artifacts may still use legacy `CC-Switch-*` file names for packaging compatibility. Treat those names as compatibility identifiers, not as the primary public product name.
...
The paths below retain legacy compatibility identifiers. See the [migration compatibility guide (ZH)](../../zh/5-faq/5.5-migration-compatibility.md).
...
**Solutions**:
1. Confirm the file was exported by bianma-app
...
**Windows**:
- Check taskbar settings to ensure the bianma-app icon is not hidden
...
**Solutions**:
1. Check network connection
2. Reinstall the latest package from your current approved distribution channel
3. If you are still using the historical Homebrew package name: `brew upgrade --cask cc-switch`

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and bianma-app runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
1. Visit [GitHub Issues](https://github.com/CreatorEdition/bianma-app-internal/issues)
...
These log directories also keep legacy compatibility identifiers. See the [migration compatibility guide (ZH)](../../zh/5-faq/5.5-migration-compatibility.md).
