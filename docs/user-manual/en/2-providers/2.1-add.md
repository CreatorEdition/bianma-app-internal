...
3. bianma-app calls the provider's `/v1/models` endpoint to retrieve the model list
...
> Authentication type is automatically detected by bianma-app (PackyCode API proxy / Google OAuth / generic API Key), no manual configuration needed.
...
bianma-app supports two ways to import provider configurations:

### Option 1: Deep Link Import

One-click import via `bianma://` protocol links. For legacy scheme and naming compatibility details, see the [migration compatibility guide (ZH)](../../zh/5-faq/5.5-migration-compatibility.md).

1. Click or visit the deep link
2. bianma-app opens automatically and shows the import confirmation
...
> **Note**: Importing will overwrite the existing database. It is recommended to export your current configuration as a backup first. Exported files may still use the legacy compatibility prefix `cc-switch-export-{timestamp}.sql`. See the [migration compatibility guide (ZH)](../../zh/5-faq/5.5-migration-compatibility.md).
