...
This section describes how to configure bianma-app according to your preferences.
...
bianma-app supports three languages:
...
When enabled, bianma-app automatically runs when the system starts.
...
When enabled, bianma-app automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that bianma-app uses when opening a terminal.
...
The storage location for bianma-app's own data, defaulting to `~/.cc-switch/`.
This path keeps a legacy compatibility identifier. See the [migration compatibility guide (ZH)](../../zh/5-faq/5.5-migration-compatibility.md).
...
Configure bianma-app's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, bianma-app automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
`cc-switch-sync` is kept only as a legacy compatibility directory name. See the [migration compatibility guide (ZH)](../../zh/5-faq/5.5-migration-compatibility.md).
...
- bianma-app automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, bianma-app displays details about the remote snapshot:
- Protocol version (v2)
...
Displays the current bianma-app version number, with support for:
